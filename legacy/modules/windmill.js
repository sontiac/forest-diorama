...
export function createWindmill(THREE, { x = 0, z = 0, facing = 0, height = 5 } = {}) {
  const group = new THREE.Group();
  const box = (w, h, d, mat) => new THREE.Mesh(new THREE.BoxGeometry(w, h, d), mat);
  const wallMat = new THREE.MeshStandardMaterial({ color: '#d9cdb6', roughness: 0.95, flatShading: true });
  ...
  // ---- Tower (tapered) ----
  const tower = new THREE.Mesh(new THREE.CylinderGeometry(0.9, 1.45, height, 14), wallMat);
