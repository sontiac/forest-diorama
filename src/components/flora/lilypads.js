...
export function createLilyPads(THREE, { center = { x: 0, z: 0 }, radius = 2.0, count = 6 } = {}) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const px = center.x + Math.cos(a) * rad;
    const pz = center.z + Math.sin(a) * rad;
    pad.position.set(px, 0.045, pz);
    pad.rotation.y = rnd() * Math.PI * 2;
    group.add(pad);
    pads.push({ pad, x: px, z: pz, baseY: 0.045, phase: rnd() * 6.28, speed: 0.6 + rnd() * 0.5, drift: (rnd() - 0.5) * 0.1 });
  }

  // Pad base positions, so other components (e.g. frogs) can sit on them.
  const positions = pads.map((p) => ({ x: p.x, z: p.z }));
